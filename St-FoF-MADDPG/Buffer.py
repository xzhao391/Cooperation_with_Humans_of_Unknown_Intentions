...
class Buffer:
    ...
    def sample(self, indices):
        ...
        obs = torch.tensor(obs, requires_grad=True).float().to(self.device)  # torch.Size([batch_size, state_dim])
        action = torch.tensor(action, requires_grad=True).float().to(self.device)  # torch.Size([batch_size, action_dim])
        reward = torch.from_numpy(reward).float().to(self.device)  # just a tensor with length: batch_size
        # reward = (reward - reward.mean()) / (reward.std() + 1e-7)
        next_obs = torch.tensor(next_obs, requires_grad=True).float().to(self.device)  # Size([batch_size, state_dim])
