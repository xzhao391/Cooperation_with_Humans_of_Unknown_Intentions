...
if __name__ == '__main__':
    ...
    parser.add_argument('--episode-num', type=int, default=80000, help='total number of episode')
    ...
    parser.add_argument('--steps-before-learn', type=int, default=1e5,
                        help='steps to be executed before agents start to learn')
    parser.add_argument('--learn-interval', type=int, default=200,
    ...
    data_actor = torch.load('results/corridor/model_actor18000.pt')
    ...
    for episode in range(args.episode_num):
            ...
            if total_step > args.steps_before_learn:
                ...
                if episode % args.save_interval == 0:
                    torch.save([agent.actor.state_dict() for agent in maddpg.agents],
                               os.path.join(model_dir, f'model_actor{episode}.pt'))
                    torch.save([agent.critic.state_dict() for agent in maddpg.agents],
                               os.path.join(model_dir, f'model_critic{episode}.pt'))
        ...
        if episode % 1000 == 0:
            if episode == 0:
                ...
                print(f'episode {episode}: cumulative reward: {np.mean(total_reward[episode-100:episode], axis = 0)}, '
                  f'sum reward: {sum(np.mean(total_reward[episode-100:episode], axis = 0))}')
